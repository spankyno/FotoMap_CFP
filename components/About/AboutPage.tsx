...
import { 
  Cpu, Map, UserCheck, UserX, Layers, Code2, Database, 
  Zap, ShieldCheck, ArrowLeft, LayoutDashboard
} from "lucide-react";
...
export const AboutPage = ({ onClose }: { onClose: () => void }) => {
  ...
  const techStack = [
    ...
    {
      title: "Auth Propio + PBKDF2",
      description: "Autenticación segura implementada con Web Crypto API nativa. Sin dependencias externas, funciona en el edge de Cloudflare.",
    ...
    {
      title: "Cloudflare D1",
      description: "Base de datos SQLite distribuida en el edge. Almacenamiento de usuarios y proyectos con latencia mínima.",
    ...
    <div className="fixed inset-0 z-[2000] bg-[#020617] overflow-y-auto text-zinc-300 font-sans selection:bg-primary/20">
      <div className="max-w-5xl mx-auto px-6 py-12 lg:py-20">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-16">
          <div>
            ...
            <h1 className="text-4xl md:text-5xl font-black text-white tracking-tight mb-2">
              Acerca de <span className="text-primary italic">FotoMap</span>
            </h1>
          ...
          <Badge variant="outline" className="px-4 py-2 border-primary/30 text-primary bg-primary/5 text-sm uppercase tracking-widest font-bold">
            Version 1.2.0
          </Badge>
        </div>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-20">
          ...
          <div className={`p-8 rounded-3xl border ${!isSignedIn ? 'border-amber-500/20 bg-amber-500/5' : 'border-zinc-800 bg-zinc-900/30'} flex flex-col`}>
            ...
            <ul className="space-y-4 mb-8 flex-1">
              ...
              <li className="flex items-start gap-3 text-sm opacity-60">
                <div className="w-5 h-5 rounded-full bg-amber-500/20 text-amber-500 flex items-center justify-center flex-shrink-0 mt-0.5">!</div>
              ...
              <li className="flex items-start gap-3 text-sm opacity-30 line-through">
                <div className="w-5 h-5 rounded-full bg-zinc-800 text-zinc-500 flex items-center justify-center flex-shrink-0 mt-0.5">✕</div>
          ...
          <div className={`p-8 rounded-3xl border ${isSignedIn ? 'border-primary/30 bg-primary/5' : 'border-zinc-800 bg-zinc-900/30'} flex flex-col`}>
            ...
            <ul className="space-y-4 mb-8 flex-1">
              ...
              <li className="flex items-start gap-3 text-sm">
                <div className="w-5 h-5 rounded-full bg-green-500/20 text-green-500 flex items-center justify-center flex-shrink-0 mt-0.5">✓</div>
                <span className="text-zinc-300">Exportación avanzada: <strong>GeoJSON, KML y Shapefile (GIS)</strong>.</span>
              </li>
              <li className="flex items-start gap-3 text-sm">
                <div className="w-5 h-5 rounded-full bg-green-500/20 text-green-500 flex items-center justify-center flex-shrink-0 mt-0.5">✓</div>
        ...
        <div className="text-center">
          <p className="text-zinc-500 text-sm mb-6 underline underline-offset-8 decoration-zinc-800">
