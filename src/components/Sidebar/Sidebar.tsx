...
import { 
  FileSpreadsheet, FileJson, Globe, Share2, Route, Flame, Settings, Layers, Archive
} from "lucide-react";
import { 
  exportToExcel, exportToGeoJSON, exportToKML, exportToShapefile, generateShareableHTML 
} from "../../utils/export";
...
export const Sidebar = () => {
  ...
  const handleExport = (type: string) => {
    ...
    if (!isSignedIn && (type === "shp" || type === "html")) {
      toast.error("Las exportaciones avanzadas (Shapefile/HTML) requieren una cuenta.");
      return;
    }
    switch (type) {
  ...
  return (
    <div className="w-80 h-full border-r border-border bg-[#1e293b] flex flex-col p-5 gap-5 overflow-y-auto z-20">
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-3 bg-card/30 p-3 rounded-lg border border-border/50">
          ...
          <div className="flex items-center justify-between">
            ...
            <Switch id="route" checked={showRoute} onCheckedChange={setShowRoute} className="scale-75" />
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3 mt-auto">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <Button variant="secondary" size="sm" className="bg-[#0f172a] border-border/50 hover:bg-[#334155] text-[11px] h-9 gap-2" onClick={() => handleExport("geojson")}>
            <FileJson className="w-3.5 h-3.5 text-sky-400" />GeoJSON
          </Button>
          <Button variant="secondary" size="sm" className="bg-[#0f172a] border-border/50 hover:bg-[#334155] text-[11px] h-9 gap-2" onClick={() => handleExport("kml")}>
            <Globe className="w-3.5 h-3.5 text-blue-400" />KML
          </Button>
          <Button variant="secondary" size="sm" className="bg-[#0f172a] border-border/50 hover:bg-[#334155] text-[11px] h-9 gap-2 relative" onClick={() => handleExport("shp")}>
            <Archive className="w-3.5 h-3.5 text-purple-400" />Shapefile
            {!isSignedIn && <div className="absolute top-0 right-0 w-2 h-2 bg-primary rounded-full -mr-1 -mt-1" />}
          </Button>
          <Button variant="default" className="col-span-2 bg-[#38bdf8] text-[#020617] hover:bg-[#38bdf8]/90 font-bold h-10 gap-2 shadow-lg shadow-sky-500/10" onClick={() => handleExport("html")}>
            <Share2 className="w-4 h-4" />Generar Mapa Compartible
          </Button>
        </div>
      </div>
    </div>
